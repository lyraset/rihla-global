...
import { SECTION_DEFAULTS, SECTION_KEYS, SPOTLIGHT_DEFAULTS } from './section-defaults.js'
...
export const COLLECTIONS = {
  sections: {
    ...
    defaultSort: { order: 1 }, columns: ['title', 'key', 'isVisible'], titleField: 'title',
    // Seeded on first visit so the editor opens showing the live copy instead
    // of an empty list the user would have to reconstruct by hand.
    ensureDefaults: {
      uniqueBy: 'key',
      // `where` is admin-only annotation, not a stored field.
      rows: SECTION_DEFAULTS.map(({ where: _where, ...row }) => row),
    },
    fields: [
      {
        name: 'key', label: 'Section', type: 'select', required: true, options: SECTION_KEYS,
        help: 'Which section on the site this copy belongs to. Changing it re-points the row.',
      },
  ...
  spotlight: {
    ...
    defaultSort: { order: 1 }, columns: ['label', 'isPublished'], titleField: 'label',
    ensureDefaults: { uniqueBy: 'label', rows: SPOTLIGHT_DEFAULTS },
    fields: [
