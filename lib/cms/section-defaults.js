...
export const PAGE_SECTIONS = {
  // The services and success sections still render here on Home — only their
  // standalone /services and /success pages were removed.
...
export function sectionCopy(sections, key, page) {
  ...
  if (!row) return { ...fallback, isVisible: true, featuredOnly: false, maxItems: 0 }
  return {
    eyebrow: row.eyebrow ?? fallback.eyebrow,
    title: row.title ?? fallback.title,
    subtitle: row.subtitle ?? fallback.subtitle,
    isVisible: row.isVisible !== false,
    featuredOnly: row.featuredOnly === true,
    maxItems: Number(row.maxItems) > 0 ? Number(row.maxItems) : 0,
  }
}

/**
 * Narrow a section's items for one page.
 *
 * Defaults are deliberately inert — no `maxItems` and `featuredOnly` off means
 * every item renders, exactly as before these controls existed. `featuredOnly`
 * is honoured only where the items actually carry the flag (services and
 * testimonials); elsewhere it would silently blank the section, so it's ignored.
 */
export function pickItems(items, cfg = {}, page) {
  let out = items || []

  // Page-scoped items win when the page has any of its own; otherwise the
  // shared set renders. That keeps a single shared list as the default (one
  // edit fixes every page) while letting any page own its wording outright.
  if (page) {
    const mine = out.filter((i) => i.page === page)
    out = mine.length ? mine : out.filter((i) => !i.page)
  } else {
    out = out.filter((i) => !i.page)
  }

  if (cfg.featuredOnly && out.some((i) => 'isFeatured' in i)) {
    const featured = out.filter((i) => i.isFeatured)
    // Never empty a section because nothing has been ticked yet.
    if (featured.length) out = featured
  }
  if (cfg.maxItems > 0) out = out.slice(0, cfg.maxItems)
  return out
}
