...
/**
 * Canonical production origin. Every emitted <loc> is absolute against this.
 *
 * Note: the apex currently serves a path-preserving 308 to www, so crawlers
 * follow one redirect per URL. That is accepted by Google and harmless; if you
 * ever want to drop the hop, either make the apex primary in the Vercel domain
 * settings or change this constant to the www host.
 */
export const CANONICAL_ORIGIN = 'https://rihla-global.com'
 ...
 */
const RESERVED_SLUGS = new Set(['about', 'contact', 'countries', 'services', 'success', 'admin', 'api'])
...
export async function getSitemapEntries() {
  ...
  const countryEntries = countries
    .filter((c) => c.code)
    .map((c) => ({
      loc: `${origin}/countries/${c.code.toLowerCase()}`,
  ...
  return [...staticEntries, ...countryEntries, ...pageEntries]
}
